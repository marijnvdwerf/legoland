...
from `./tools/verify` (aggregate today: **52.71%** across 3392 functions). "MATCHED"
...
```
_WinMainCRTStartup (CRT)                         [TU_CRT, out of denominator]
└─ WinMain (thunk, debug.c)              0x------  (synthetic, not annotated)
   └─ WinMainBody                        0x0047fd10  debug.c   MATCHED 100%
      ...
      └─ FUN_0047f880  (main init+loop) 0x0047f880  debug.c   MATCHED 99.62%
         ...
         ├─ FUN_00498d00
         ├─ InitHostSystemGPU           0x00463700  draw.c    STUB  *** DDraw device glue — SHIM ***
         ├─ InitScreen                  0x00463870  draw.c    STUB  *** DDraw surfaces+window — SHIM ***
         ├─ InitInputSystem             0x00473870  input.c   STUB  *** DInput device glue — SHIM ***
         ...
         └─ FUN_00459520  (game orchestrator + frame-loop driver)  0x00459520  screens.c  STUB
```
...
## 2. The frame loop and how it dispatches to the current screen
...
- **dispatches on the global screen-mode `DAT_008119b4`:**
...
This is the **title screen's per-frame render + input dispatch**. Sequence:
`ResetHitInfo` (100%) → `PushRenderingStatusAndLockVideoSurface` (STUB/shim) →
...
**`UpdateFocussedIconPtr`** (MATCHED 100%) → `PopRenderingStatus` (STUB/shim) →
...
Grouped by subsystem; **critical-path blockers first**. "size/cx" is a rough estimate
from the Ghidra decompile (S ≤ ~30 lines, M ~30–80, L > 80, XL > 150). "callees" notes
whether the function's own callees are already satisfied.

### A. Frame loop / screen dispatch  (THE backbone — do first)
| addr | name | file | size/cx | callees | notes |
|---|---|---|---|---|---|
| `0x00458c00` | `FUN_00458c00` | screens.c | L | most MATCHED; calls `ReadGameButtons`(B), `InitScreens`(A), `FUN_00458ee0`(in-game, can stay STUB for title), `FUN_00459360`(STUB), `FUN_004771f0`(OOS) | **single per-frame tick + state switch. Top blocker.** |
| `0x00458640` | `InitScreens` | screens.c | M | `InitTitleScreen`(C), `RenderFrontEndScreen`(below), other screen inits can stay STUB | front-end dispatcher |
| `0x00458740` | `RenderFrontEndScreen` | screens.c | M | needs `PrintSprite`(E), `RenderIcons`(D), `CheckFocussedIcon`(D), `PrintExitCheckBox`(F), `ProcessFrontEndHelp`(F), `RenderingComplete`(E/shim) | **per-frame title render** |
| `0x00459520` | `FUN_00459520` | screens.c | XL | many MATCHED; `InitSoundSystem`(G), `SetupControllers`(B), `LLIDB_*`, `FUN_004771f0`(OOS, ifdef) | orchestrator that *enters* the loop; large but mostly sequential calls |
...
| `0x00473b00` | `UpdateControllerFromMouseData` | input.c | M | DInput mouse state → `CONTROLLERBUFFER` | fills the buffer `ReadGameButtons` consumes |
| `0x00473c10` | `UpdateControllerFromKeyboardData` | input.c | M | keyboard state → `CONTROLLERBUFFER` | same |
| `0x004738b0` | `FUN_004738b0` | input.c | M | DInput create mouse device | sub of `InitInputSystem`(shim) — see §4 |
| `0x00473970` | `FUN_00473970` | input.c | M | DInput create keyboard device | sub of `InitInputSystem`(shim) — see §4 |
| `0x004740b0` | `GetInputChar` | input.c | S | keyboard buffer → char | needed only for name-entry popups, not bare title |

### C. Title UI load
| addr | name | file | size/cx | callees | notes |
|---|---|---|---|---|---|
| `0x0048fc40` | `InitTitleScreen` | title.c | L | `LoadSprite`(100%), `LoadSpriteIcon`(100%), `GetString`(100%), `FUN_0048fc30`, handlers already in title.c | **builds the title menu**: loads `TitleScreenBk.lls` + 7 menu icons (New/Free/Reg/Exit/Load/MovieOn) and wires each icon's `event_handler`. Logic is self-contained; the heavy lifting (`LoadSpriteIcon`) is MATCHED. **High leverage.** |
...
| `0x0046f4c0` | `CheckFocussedIcon` | icon.c | M | dispatches `FocussedIconPtr->event_handler` and the default `DAT_006687bc/c0` handlers using `ReadGameButtons` bits | **routes mouse/keys to the menu button handlers — the "responds to input" core.** |
...
### E. Render — sprite raster + present (mixed: dispatch is real game code, raster is shim)
| addr | name | file | size/cx | callees | notes |
|---|---|---|---|---|---|
| `0x004853a0` | `PrintSprite` | print_sprite.c | L | `GetVRAMAddress`(100%), `FUN_00499500`(100%), `RenderSprite`(E/shim), `RenderSpriteX`(E/shim) | **sprite draw dispatch + hit-info writeback (`DAT_004bdd00`).** The dispatch/hit logic is real game code worth matching; it calls the raster primitives below. |
...
| `0x00466500` | `RenderingComplete` | draw.c | S | `ProcessSystemEvents`(B), `PTR_FUN_004b9ca4`→`FUN_004661d0`(shim blit), `rdtsc`, `GetTickCount` | **the per-frame "present" + input pump.** Small; logic is real, but the actual flip is the shim `FUN_004661d0`. |

### F. Title overlays / help (only if you want the full title behavior)
| addr | name | file | size/cx | notes |
|---|---|---|---|---|
...
| addr | name | file | size/cx | notes |
|---|---|---|---|---|
| `0x004964f0` | `InitSoundSystem` | sound_music.c | M | DirectSound init; **port-replaceable** — but it's called in `FUN_00459520`. If kept real it pulls DSound; otherwise shim it. `PlayInstanceOfSample` (the click SFX) is already MATCHED. |

---

## 4. Port-replaceable shims (link-stub only — DO NOT byte-match)
...
**link** (keep `STUB()`), not reach 100%. They are on the boot/critical path but are
...
| `0x004964f0` | `InitSoundSystem` | sound_music.c | DirectSound init (if kept as shim) |
...
> the raw device-create/lock/blit calls are shims. Similarly `PrintSprite`'s dispatch is
> in scope while the `RenderSprite*` rasterizers underneath are shims.
...
| `0x00490090/d0/110` | per-region movie buttons (title.c) | already real, but invoke the OOS movie player |

---

## 5. Counts and highest-leverage next targets

**In-scope STUBs that block an interactive title screen** (subsystems A–F above, excluding
the §4 shims and OOS movies): **~22 functions**, concentrated in:
`screens.c` (frame loop + dispatch: `FUN_00458c00`, `InitScreens`, `RenderFrontEndScreen`,
...
The ~16 §4 shims and ~5 movie functions are **not** in that count (link-stubs / OOS).

### Top 10 highest-leverage next targets (do in this order)
...
4. **`InitTitleScreen` `0x0048fc40`** (title.c) — builds the menu; its heavy callees (`LoadSpriteIcon`, `GetString`) are already MATCHED, so high yield.
...
6. **`CheckFocussedIcon` `0x0046f4c0`** (icon.c) — dispatches input to the menu button handlers (which are already mostly real).
7. **`RenderIcons` `0x0046eee0`** (icon.c) — draws the menu + focus highlight.
8. **`PrintSprite` `0x004853a0`** (print_sprite.c) — sprite draw + hit-info writeback used by both bg and icons (raster underneath is shimmed).
